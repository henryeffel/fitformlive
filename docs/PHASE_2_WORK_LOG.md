...
- OpenCV.js 파일 크기가 약 10MB이므로 정적 호스팅 압축과 캐시 설정이 필요하다.

## 2026-07-28 로컬 고정 후속 조치

실제 브라우저 테스트에서 기존 OpenCV 공식 문서 URL이 HTTP 403으로 실패해 `OpenCV.js: unavailable` 상태가 발생했다.

핵심 MoveNet 추론과 팔 컬 카운트는 오류 격리 덕분에 계속 동작했지만 밝기와 선명도 값은 측정되지 않았다.

다음과 같이 수정했다.

- `@techstark/opencv-js@4.10.0-release.1`의 브라우저 배포 파일 사용
- `web/vendor/opencv-4.10.0.js`로 로컬 고정
- Apache License 2.0 사본 저장
- 다운로드 출처, 버전 및 SHA-256을 `web/vendor/NOTICE.md`에 기록
- HTML이 동일 출처의 로컬 OpenCV.js를 비동기로 로드하도록 변경

고정 파일 SHA-256:

```text
19B46167B59EFBEF9CC3836264B0B657110833E4B1DF84282004B8B3141C048D
```

수정 후 브라우저를 강력 새로고침하고 `OpenCV.js: ready`, `brightness`, `sharpness`가 표시되는지 다시 확인해야 한다.

### 동작 인식 우선순위 보완

로컬 고정 후에는 이전에 비활성화됐던 OpenCV 품질 분석이 실제로 실행된다.
이때 품질 분석이 MoveNet 추론보다 먼저 실행되고 촬영 환경 경고가 운동 피드백을 덮어써,
사용자가 동작 인식이 멈춘 것으로 오해하거나 저사양 장치에서 추론 주기가 흔들릴 수 있었다.

다음과 같이 핵심 자세 인식을 품질 분석보다 우선하도록 수정했다.

- MoveNet 추론과 카운트 처리가 끝난 후 OpenCV 품질 분석 실행
- 분석 간격을 500ms에서 1000ms로 조정
- 분석 너비를 320px에서 240px로 조정
- 품질 경고가 운동 단계의 라벨과 성공 상태를 덮어쓰지 않고 상세 안내에만 추가되도록 변경

OpenCV는 보조적인 촬영 환경 진단 역할만 하며, 운동 인식과 카운트가 항상 우선한다.
